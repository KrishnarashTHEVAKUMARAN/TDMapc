...
public class MacDeauRestaurant implements BurgerRestaurant {
    ...
    public BurgerCook order_menu(MenuType menu) {
        ...
        switch (menu) {
            case MEAT_MENU:
                menuBurger = new McDeauCook("Beefy", Size.MEDIUM, MeatType.BEEF).with_sauce(SauceType.BURGER).with_onions().with_cheese();
                break;
            case FISH_MENU:
                menuBurger = new McDeauCook("Fishy", Size.SMALL, MeatType.WHITEFISH).with_sauce(SauceType.BEARNAISE);
                break;
            case CHEESE_MENU:
            default:
                menuBurger = new McDeauCook("Cheesy", Size.SMALL, MeatType.BEEF).with_cheese();
        }
        return menuBurger;
    }
