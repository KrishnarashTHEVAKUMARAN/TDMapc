...
public class BigBurgerRestaurant implements BurgerRestaurant {
    ...
    public BurgerCook order_menu(MenuType menu) {
        BurgerCook menuBurger = null;
        switch (menu) {
            case MEAT_MENU:
                menuBurger = new BigBurgerCook("BigMarcel", Size.BIG, MeatType.BEEF).with_sauce(SauceType.BURGER).with_onions().with_tomato();
                break;
            case FISH_MENU:
                menuBurger = new BigBurgerCook("FiletO'Poisson", Size.MEDIUM, MeatType.WHITEFISH).with_sauce(SauceType.BEARNAISE);
                break;
            case CHEESE_MENU:
            default:
                menuBurger = new BigBurgerCook("MaxeeCheesee", Size.MEDIUM, MeatType.BEEF).with_cheese().with_onions();
        }
        return menuBurger;
    }
